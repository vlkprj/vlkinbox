...
//сабміт кінець//
    ...
    if (doorBtn) {
       ...
       const noArtifactsText = "\n\n👀 Ви зможете додавати в додаток власні артефакти починаючи з 3 рівня";
        ...
        const getArtifactText = (count) => `\n\n📸 **Тепер ви можете додавати власні артефакти у додаток! Вам доступно: ${count} шт.**
Сфоткай у Валках щось: стару табличку, декоративний елемент, дорожній знак або цеглину, красивий чи не дуже красивий кущ, якийсь напис десь на стіні, лавку, деревце, щось знайдене по дорозі, або якийсь дивний предмет; кота, урну або інший об’єкт валківської цивілізації. 

Важливо: не фоткай військову техніку чи якісь обʼєкти критичної інфраструктури, ми тут просто розважаємось, єслі шо. Людей теж не фотографуй. Ми таке не приймаємо і не додаємо в додаток. Тільки маленькі деталі міста: котик по дорозі, кирпичик відпавший від будівлі, красива квітка/міні-клумба, кульок, який застряг на дереві, знак, напис, табличка, пеньок, чийсь загублений ключ, короче кажучи, будь-який цікавий або навпаки — супернецікавий предмет або невеличкий обʼєкт, який ви побачили у місті. Встигніть додати найкрутіші і найбільші артефакти, кожен має бути унікальним! 

Обовʼязково опиши що це, де і коли ти це побачив(ла) — і відправляй скріншот цієї картки та фото сюди: @valkyshobot 👈🏻 Після перевірки ми додамо твій артефакт, і тепер хтось інший буде за ним полювати 🔍`;
